...
// Helper function to auto-generate components from PhysicalCopy flags
function generateComponentsFromPhysicalCopy(release: any): Component[] {
  const components: Component[] = [];

  // Slipcover
  if (release.hasSlipcover) {
    components.push({
      type: 'slipcover',
      name: 'Slipcover',
      required: true,
  ...
  // Bonus discs
  if (release.hasBonusDisc) {
    const count = release.bonusDiscCount || 1;
  ...
  if (release.packageType) {
    const pkgType = release.packageType.toLowerCase();
  ...
  // Merge with existing components JSON if present
  if (release.components && typeof release.components === 'object') {
    // The components field is a JSON field that might have packageTypes, includedItems, etc.
    // We'll preserve this structure
  }
...
export const getPhysicalCopy = asyncHandler(async (req: Request, res: Response) => {
  ...
  const autoGeneratedComponents = generateComponentsFromPhysicalCopy(copy);
